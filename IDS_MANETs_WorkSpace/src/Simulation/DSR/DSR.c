...
_declspec(dllexport) int fn_NetSim_DSR_Metrics(PMETRICSWRITER filename)
{
	...
	{
		add_table_row_formatted(false, table, "%d,%f,%f,",mal_devid[i], mal_start_time[i],mal_det_time[i]);
	}
	PMETRICSNODE menu = init_metrics_node(MetricsNode_Menu,"IDS_METRICS", NULL);
